        ...
        >
          <CarouselContent>
            {brands.map((brand, index) => {
              // Extract image URL matching API structure (image_url)
              const imageUrl =
                typeof brand === "string"
                  ? brand
                  : brand?.image_url || brand?.media_url || brand?.image || brand?.logo || "";

              // Extract fallback display text using slug
              const brandName =
                typeof brand === "string"
                  ? brand
                  : brand?.name || brand?.slug || `brand-${index}`;

              return (
                <CarouselItem
                  key={typeof brand === "object" && brand?.slug ? brand.slug : index}
