...
export default function CategorySection(props: CategorySectionProps) {
  ...
  if (isLoading) {
    return (
      ...
      //         <div
      //           key={i}
      //           className="flex-shrink-0 w-[42%] sm:w-[28%] md:w-[20%] lg:w-[14%] bg-white rounded-2xl p-2 border border-gray-100 shadow-sm flex flex-col items-center gap-2"
      //         >
      //           {/* Image Skeleton */}
      //           <Skeleton className="w-full h-28 md:h-32 rounded-full" />
      //           {/* Text Skeleton */}
      //           <Skeleton className="h-4 w-3/4 rounded-md my-1" />
      //         </div>
      //       ))}
      //     </div>
      ...
      <div className="container mx-auto px-4 md:px-8 relative">
          ...
          {!error &&
            categories.map((item, index) => {
              ...
              return (
                ...
                >
                  <div className="bg-background rounded-2xl p-2 border border-gray-100 shadow-sm transition-all flex flex-col items-center text-center">
                    {/* Category Image */}
                    <Link href={`/category/${item.slug}`} className="w-full">
                      <div className="w-full h-28 md:h-32 bg-background rounded-full flex items-center justify-center">
                        <img
                          src={imgSrc}
                          alt={item.name}
                          className="max-h-full rounded-full max-w-full object-contain hover:scale-105 transition-transform"
                        />
                      </div>
                    </Link>
        ...
        {activeItem &&
          ...
          activeItem.category.children.length > 0 && (
            ...
            >
              <div className="flex gap-2 p-2.5 rounded-xl shadow-2xl border border-gray-200 min-w-max bg-white">
                {activeItem.category.children.map((child) => (
                  ...
                  >
                    <div className="rounded-lg flex items-center justify-center border border-gray-100">
                      <img
                        src={
                          child.image ||
                          child.image_url ||
